...
public final class BoundedBuffer<T> implements Buffer<T> {
    ...
    public boolean add(T elem, long millis) throws InterruptedException {
        boolean acquired = putSema.tryAcquire(millis, TimeUnit.MILLISECONDS);
    ...
    public T remove(long millis) throws InterruptedException {
        boolean acquired = takeSema.tryAcquire(millis, TimeUnit.MILLISECONDS);
    ...
    @Override
    public int size() {
        return this.takeSema.availablePermits();
    }
}
