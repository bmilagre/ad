...
/**
 * Produzent, der eine maximale Anzahl Werte produziert und diese in eine BlockingQueue speichert.
 */
public final class BlockingQueueProducer implements Callable<Long> {
     ...
     */
    public BlockingQueueProducer(final BlockingQueue<Integer> queue, final int max) {
        this.queue = queue;
        this.maxRange = max;
    }
    ...
    public Long call() throws Exception {
        ...
        for (int i = 1; i <= maxRange; i++) {
            sum += i;
            queue.put(i); // Blockiert, wenn die Queue voll ist
        }
        return sum;
    }
}